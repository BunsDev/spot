...
task("ops:info")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const latestBond = await hre.ethers.getContractAt("BondController", await bondIssuer.callStatic.getLatestBond());

    const feeStrategy = await hre.ethers.getContractAt("BasicFeeStrategy", await perp.feeStrategy());
    const pricingStrategy = await hre.ethers.getContractAt("UnitPricingStrategy", await perp.pricingStrategy());
    const depositBond = await hre.ethers.getContractAt("BondController", await perp.callStatic.getDepositBond());
    ...
    console.log("---------------------------------------------------------------");
    console.log("TrancheQueue:");
    const queueCount = (await perp.callStatic.getRedemptionQueueCount()).toNumber();
    const queueTranches: { [key: string]: boolean } = {};
    console.log("redemptionTranche:", await perp.callStatic.getRedemptionTranche());
    console.log("queueCount:", queueCount);
    for (let i = 0; i < queueCount; i++) {
      const t = await perp.callStatic.getRedemptionQueueAt(i);
      queueTranches[t] = true;
      console.log(`queue(${i}):`, t);
    }

    console.log("---------------------------------------------------------------");
    console.log("Reserve:");
    const reserveCount = (await perp.reserveCount()).toNumber();
    let reserveValue = BigNumber.from("0");
    console.log("reserveCount:", reserveCount);
    for (let i = 0; i < reserveCount; i++) {
      const tranche = await hre.ethers.getContractAt("Tranche", await perp.reserveAt(i));
      const balance = await tranche.balanceOf(perp.address);
      const value = await perp.perpsToTranches(tranche.address, balance);
      reserveValue = reserveValue.add(value);
      console.log(
        `reserve(${i}):${tranche.address}`,
        `\t${queueTranches[tranche.address] === true ? "queue" : "icebox"}`,
        `\t${utils.formatUnits(balance, await tranche.decimals())}`,
        `\t${utils.formatUnits(value, await perp.decimals())}`,
      );
    }
    console.log("ReserveValue:", utils.formatUnits(reserveValue, await perp.decimals()));
    console.log("TotalSupply:", utils.formatUnits(await perp.totalSupply(), await perp.decimals()));
  });
...
task("ops:trancheAndDeposit")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const perp = await hre.ethers.getContractAt("PerpetualTranche", perpAddress);
    const bondIssuer = await hre.ethers.getContractAt("BondIssuer", await perp.bondIssuer());
    const collateralToken = await hre.ethers.getContractAt("ButtonToken", await bondIssuer.collateralToken());
    ...
    const deployerAddress = await (await hre.ethers.getSigners())[0].getAddress();
...
task("ops:redeem")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const [burnAmt, , burnFee, tranchesRedeemed] = await router.callStatic.previewRedeemFromQueue(
      perp.address,
      fixedPtAmount,
      constants.MaxUint256,
    );
    const requestedTranches = tranchesRedeemed.filter((t: string) => t !== constants.AddressZero);
    console.log("burnAmt", utils.formatUnits(burnAmt, await perp.decimals()));
    console.log("burnFee", utils.formatUnits(burnFee, await feeToken.decimals()));
    console.log("tranches redeemed");
    for (let i = 0; i < requestedTranches.length; i++) {
      console.log(`tranches(${i}):`, requestedTranches[i]);
    }
    ...
    console.log("Redeem:");
    const tx3 = await router.redeem(perp.address, fixedPtAmount, fee, requestedTranches);
    await tx3.wait();
    console.log("Tx", tx3.hash);
...
task("ops:redeemIcebox")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const reserveCount = (await perp.reserveCount()).toNumber();
    const queueTranches: { [key: string]: boolean } = {};
    const iceboxTranches = [];
    const iceboxTrancheBalances = [];
    for (let i = 0; i < queueCount; i++) {
      const t = await perp.callStatic.getRedemptionQueueAt(i);
      queueTranches[t] = true;
    }
    for (let i = 0; i < reserveCount; i++) {
      const tranche = await hre.ethers.getContractAt("Tranche", await perp.reserveAt(i));
      if (queueTranches[tranche.address] !== true) {
        iceboxTranches.push(tranche);
        iceboxTrancheBalances.push(await tranche.balanceOf(await perp.reserve()));
        console.log(`icebox(${iceboxTranches.length - 1}):${tranche.address}`);
      }
    }
    if (iceboxTranches.length === 0) {
      console.log("icebox empty");
    }

    console.log("---------------------------------------------------------------");
    console.log("Preview redeem:", amount);
    const [burnAmt, , burnFee, tranchesRedeemed] = await router.callStatic.previewRedeemFromIcebox(
      perp.address,
      fixedPtAmount,
      iceboxTranches.map(t => t.address),
    );
    const requestedTranches = tranchesRedeemed.filter((t: string) => t !== constants.AddressZero);
    console.log("burnAmt", utils.formatUnits(burnAmt, await perp.decimals()));
    console.log("burnFee", utils.formatUnits(burnFee, await feeToken.decimals()));
...
task("ops:redeemTranches")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const totalIssued = await bondIssuer.callStatic.totalIssued();
    for (let i = 0; i < totalIssued; i++) {
      ...
      for (let j = 0; j < trancheCount; j++) {
        ...
        const tranche = await hre.ethers.getContractAt("Tranche", address);
...
task("ops:trancheAndRollover")
  ...
  .setAction(async function (args: TaskArguments, hre) {
    ...
    const depositBond = await hre.ethers.getContractAt("BondController", depositBondAddress);

    console.log("---------------------------------------------------------------");
    console.log("Preview tranche:", collateralAmount);
    const depositTranches = [];
    for (let i = 0; i < trancheAddresses.length; i++) {
      const tranche = await hre.ethers.getContractAt("Tranche", trancheAddresses[i]);
    ...
    const remainingTrancheOutAmts: BigNumber[] = iceboxTrancheBalances.map(b => b);

    const rolloverData: any[] = [];
    for (let i = 0, j = 0; i < depositTranches.length && j < iceboxTranches.length; ) {
      const trancheIn = depositTranches[i];
      const trancheOut = iceboxTranches[j];
      const [rd, , rolloverFee] = await router.callStatic.previewRollover(
        perp.address,
        trancheIn.address,
        trancheOut.address,
        remainingTrancheInAmts[i],
        remainingTrancheOutAmts[j],
      );

      const trancheInAmt = remainingTrancheInAmts[i].sub(rd.remainingTrancheInAmt);
      if (rd.rolloverAmt.gt("0")) {
        rolloverData.push({
          trancheIn: trancheIn,
          trancheOut: trancheOut,
          trancheInAmt,
          trancheOutAmt: rd.trancheOutAmt,
        });

        totalRolloverAmt = totalRolloverAmt.add(rd.rolloverAmt);
        totalRolloverFee = totalRolloverFee.add(rolloverFee);
      }

      remainingTrancheInAmts[i] = rd.remainingTrancheInAmt;
      remainingTrancheOutAmts[j] = remainingTrancheOutAmts[j].sub(rd.trancheOutAmt);

      if (remainingTrancheInAmts[i].eq("0")) {
        i++;
      }
      if (remainingTrancheOutAmts[j].eq("0")) {
        j++;
      }
    }
    ...
    console.log(rolloverData.map(r => [r.trancheIn.address, r.trancheOut.address, r.trancheInAmt]));
    const tx3 = await router.trancheAndRollover(
      perp.address,
      depositBond.address,
      fixedPtCollateralAmount,
      rolloverData.map(r => [r.trancheIn.address, r.trancheOut.address, r.trancheInAmt]),
      fee,
    );
    await tx3.wait();
    console.log("Tx", tx3.hash);
